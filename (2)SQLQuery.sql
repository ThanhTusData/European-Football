...
--Làm sạch dữ liệu
--Xóa các cột không cần thiết trong bảng Match
...
--Right Join Player và Player_Attributes
SELECT
    p.player_api_id,
    p.player_name,
    ...
    -- Thay giá trị NULL của preferred_foot bằng 'right'
    CASE 
        WHEN pa.preferred_foot IS NULL THEN 'right'
        ELSE pa.preferred_foot
    END AS preferred_foot,
    -- Thay giá trị NULL hoặc không hợp lệ trong attacking_work_rate và defensive_work_rate
    CASE 
